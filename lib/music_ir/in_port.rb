    ...
    def blocking_read
      ...
      while event.nil?
        ...
        if raw_events
          ...
          when MusicIR::Event::NOTE_ON
            event = MusicIR::NoteOnEvent.new({
                      :pitch     => raw_event[:message][1],
                      :velocity  => raw_event[:message][2],
                      :timestamp => raw_event[:timestamp],
                    })
          when MusicIR::Event::NOTE_OFF
            event = MusicIR::NoteOffEvent.new({
                      :pitch     => raw_event[:message][1],
                      :velocity  => raw_event[:message][2],
                      :timestamp => raw_event[:timestamp],
                    })
          when MusicIR::Event::META
            case ((raw_event[:message][0]*256) + raw_event[:message][1])
            when MusicIR::Event::KEY_SIG
              raise RuntimeError.new("parsing error") if raw_event[:message][2] != 2
              event = MusicIR::KeySigEvent.new({
