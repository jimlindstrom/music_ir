...
describe MusicIR::BeatPosition do
  ...
  context "num_values" do
    it "should return 360" do
      MusicIR::BeatPosition.num_values.should == 360
    end
  end
  ...
  context "+" do
    it "should take a BeatPosition and a Duration and return a new BeatPosition" do
      ...
      d = MusicIR::Duration.new(9)
      (b + d).should be_an_instance_of MusicIR::BeatPosition
    end
    it "should add the Duration to the BeatPosition" do
      m = MusicIR::Meter.new(4, 4, 2) # 4/4 in eighth notes
      b = m.initial_beat_position
      d = MusicIR::Duration.new(2+1) # 3 eighth notes
      x = b + d
    ...
    it "should add the Duration to the BeatPosition, wrapping measures as needed" do
      m = MusicIR::Meter.new(4, 4, 2) # 4/4 in eighth notes
      b = m.initial_beat_position
      d = MusicIR::Duration.new(8+4+1) # 3 eighth notes
      x = b + d
