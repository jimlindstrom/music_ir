...
describe MusicIR::Pitch do
  context "new" do
    it "should take an integer from 0 to 127" do
      MusicIR::Pitch.new(0).should be_an_instance_of MusicIR::Pitch
    end
    it "should take an integer from 0 to 127" do
      MusicIR::Pitch.new(127).should be_an_instance_of MusicIR::Pitch
    end
    it "raise an error on integers < 0" do
      expect { MusicIR::Pitch.new(-1) }.to raise_error(ArgumentError)
    end
    it "raise an error on integers > 127" do
      expect { MusicIR::Pitch.new(128) }.to raise_error(ArgumentError)
    end
  end

  context "num_values" do
    it "should return 128" do
      MusicIR::Pitch.num_values.should == 128
    end
  end
