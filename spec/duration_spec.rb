...
describe MusicIR::Duration do
  ...
  context "new" do
    it "should take an integer from 0 to 127" do
      MusicIR::Duration.new(0).should be_an_instance_of MusicIR::Duration
    end
    it "should take an integer from 0 to 127" do
      MusicIR::Duration.new(127).should be_an_instance_of MusicIR::Duration
    end
    it "raise an error on integers < 0" do
      expect { MusicIR::Duration.new(-1) }.to raise_error(ArgumentError)
    end
    it "raise an error on integers > 127" do
      expect { MusicIR::Duration.new(128) }.to raise_error(ArgumentError)
    end
  end
